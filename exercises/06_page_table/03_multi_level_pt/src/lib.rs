...
impl Sv39PageTable {
    ...
    pub fn map_page(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        for level in (1..=2).rev() {
            ...
            let next_ppn = if pte & PTE_V == 0 {
                let intermediate_ppn = self.alloc_node();
                self.nodes.get_mut(&curr_ppn).unwrap().entries[vpn] = (intermediate_ppn << PPN_SHIFT) | PTE_V;
                intermediate_ppn
            } else {
                (pte >> PPN_SHIFT) & ((1 << 44) - 1)
            };
            curr_ppn = next_ppn
        }
        let vpn_0 = Self::extract_vpn(va, 0);
        self.nodes.get_mut(&curr_ppn).unwrap().entries[vpn_0] = (target_ppn << PPN_SHIFT) | flags;
    }
    ...
    pub fn map_superpage(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        let pte_2 = self.nodes.get(&curr_ppn).unwrap().entries[vpn_2];
        let ppn_1 = if pte_2 & PTE_V == 0 {
            let new_ppn = self.alloc_node();
            self.nodes.get_mut(&curr_ppn).unwrap().entries[vpn_2] = (new_ppn << PPN_SHIFT) | PTE_V;
            new_ppn
        } else {
            (pte_2 >> PPN_SHIFT) & ((1 << 44) - 1)
        };
        let vpn_1 = Self::extract_vpn(va, 1);
        self.nodes.get_mut(&ppn_1).unwrap().entries[vpn_1] = (target_ppn << PPN_SHIFT) | flags;
    }
}
