...
impl FlagChannel {
    ...
    pub fn consume(&self) -> u32 {
        ...
        loop {
            if self.ready.load(Ordering::Acquire) {return self.data.load(Ordering::Relaxed);}
        }
    }
...
impl OnceCell {
    ...
    pub fn init(&self, val: u32) -> bool {
        ...
        if self.initialized.load(Ordering::Relaxed) {return false;}
    ...
    pub fn get(&self) -> Option<u32> {
        ...
        if self.initialized.load(Ordering::Relaxed) {return Some(self.value.load(Ordering::Acquire));}
        None
    }
}
